...
<div id="admin">
    ...
    <!--    <main id="withdraw" class="moneyoperations">-->
    ...
    <!--                <input type="number" id="withdrawamount" placeholder="Сумма $0.00">-->
    ...
    <!--                    </a>-->
    <!--                </div>-->
    <!--                <button type="submit" id="submitwithdraw" class="helpbutton">Вывод средств</button>-->
    <!--            </form>-->
    <!--        </div>-->
...
<script>
    ...
    let submit = document.getElementById('submitwithdraw');
    message('', 'off');
    submit.addEventListener('click', function (e) {
        e.preventDefault();
        let input = document.getElementById('withdrawamount');
        message('', 'off')
        input.value < 25 ? message('Минимальная сумма для вывода 1000 грн', 'alert') : '';

        input.value > balance ? message('Не хватает средств', 'warning') : '';
        //todo выбран ли метод
        if (input.value >= 25 && input.value <= balance) {
            message('Заявка успешно создана', 'ok')
        }
    })
</script>
</body>
